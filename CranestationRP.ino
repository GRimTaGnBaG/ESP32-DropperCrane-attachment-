...
void handleRoot() {
  ...
  page += "<button onclick=\"sendCommand('c')\" style=\"background-color: white; color: red; width: 30%; height: 100px; margin: 5px;\">Cut</button>";  
  page += "<button onclick=\"sendCommand('p1')\" style=\"background-color: white; color: black; width: 30%; height: 100px; margin: 5px;\">POD1</button>";
  page += "<button onclick=\"sendCommand('p2')\" style=\"background-color: white; color: black; width: 30%; height: 100px; margin: 5px;\">POD2</button>";
  page += "</div>";
  page += "<script>"
...
void handleCommand() {
  ...
  if (server.hasArg("cmd")) {
    ...
    if (cmd == "u") {
      ...
      command = "AT+SEND=1,1,c"; // Cut    
    } else if (cmd == "p1") {
      command = "AT+SEND=1,2,p1 "; // POD 1
    } else if (cmd == "p2") {
      command = "AT+SEND=1,2,p2 "; // POD 2
    }  
    if (!command.isEmpty()) {
      Serial.println(command);
